...
class PriorityFeeManager:
    ...
    async def _fetch_priority_fees_from_rpc(self) -> list:
        ...
        # These would come from actual RPC call in production
        return [
    ...
    def set_extra_percentage(self, percentage: float):
        ...
        Args:
            percentage: Extra percentage (0.1 = 10%)
        """
...
async def main():
    ...
    dynamic_manager = PriorityFeeManager(
        ...
        extra_percentage=0.15,  # 15% extra
        hard_cap=500_000
    )
    ...
    print("\n🔄 Strategy Switching:")
    dynamic_manager.set_strategy(FeeStrategy.FIXED)
    dynamic_manager.set_fixed_fee(200_000)
    
    new_fee = await dynamic_manager.calculate_priority_fee()
    print(f"   New fee after switch: {new_fee:,} lamports")
